...
namespace Pokemod.Common.Systems
{
    ...
    public class ModIntegrationsSystem : ModSystem
    {
        ...
        private void DoBossChecklistIntegration()
        {
            ...
            string internalName = "MalformedSubject";

            // Value inferred from boss progression, see the wiki for details
            float weight = 19f;

            // Used for tracking checklist progress
            Func<bool> downed = () => DownedBossSystem.downedMewtwoBoss;

            // The NPC type of the boss
            int bossType = ModContent.NPCType<Content.NPCs.Bosses.Mewtwo.MewtwoBossBody>();

            // The item used to summon the boss with (if available)
            int spawnItem = ModContent.ItemType<Content.Items.Consumables.BossItems.MewtwoBossSummonItem>();

            // "collectibles" like relic, trophy, mask, pet
            List<int> collectibles = new List<int>()
            {
                ModContent.ItemType<Content.Items.GeneticSamples.PerfectedSample>(),
                ModContent.ItemType<Content.Items.GeneticSamples.MalformedSample>(),
                ModContent.ItemType<Content.Items.Consumables.BossItems.MewtwoBossBag>(),
                ModContent.ItemType<Content.Tiles.BossTrophies.MewtwoBossRelicItem>(),
                ModContent.ItemType<Content.Tiles.BossTrophies.MewtwoBossTrophyItem>()
            };
